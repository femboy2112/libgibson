...
| Sub-cell glyph realization / fallback ladder | IMPLEMENTED + TESTED, Rust-only | `SubcellGlyphMode` Braille2×4→HalfBlock1×2→Block→ASCII; `transcode_surface_glyphs` re-realizes a Braille surface (Braille2×4 = identity, so the hero path is byte-identical). Explicit policy/override (`--glyphs=` / `LIBGIBSON_GLYPHS` / Auto; console-safe on `TERM=linux`) — NOT font-repertoire detection. Orthogonal to color; no C ABI added. See [GLYPHS.md](GLYPHS.md). |
