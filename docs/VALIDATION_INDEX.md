...
| Crossterm #1126 fix analysis | [Fix analysis](CROSSTERM_1126_FIX_ANALYSIS.md) | Corroborated defect (early-return over a shared mio readiness batch), the drain-fix's blocking-fd hang reproduced by strace, two proposed fix shapes (A: single-read discipline, B: non-blocking fd) | Diagnosis corroborated, no production fix implemented; no vendored crossterm fork or `[patch.crates-io]`; #15 remains open. |

| Runtime Observatory / grounded modes | [Runtime Observatory](RUNTIME_OBSERVATORY.md) | Million-Tick drives a real `StoryDirector` under `TraceRetention` (#10); Ghost-Key replays the real `docs/fixtures/event-pressure/{crossterm,raw}-gate.tsv` + syscall witness (#15); Ownership-Duel replays a newly captured real lease-transition fixture (`docs/fixtures/ownership/duel.tsv`, captured via `script` over a PTY, #11); shared bounded diagnostic log and `?`-for-unobserved epistemic rule | `--dump`-only diagnostic instrument, no live loop, three heavier modes (Slow-Terminal, Restore-Failure, Endurance) not built; does not close #10, #11 or #15; VmRSS/VmHWM are Linux `/proc`-specific; #11's `restore()` error path is implemented but not covered by a failing-write test. |
