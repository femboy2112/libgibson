...
**Status:** RESEARCH / PRESSURE TEST ONLY  
**Date:** 2026-09-24  
...
[
oxed{	ext{information structure} ightarrow 	ext{interaction geometry}}
]
...
[
	ext{task state} ightarrow 	ext{representation choice} ightarrow 	ext{temporary interactive instrument}
]
...
After the IR/API is frozen, create fresh holdouts such as:
...
These must be selected **after** the interface is frozen.
...
[
	ext{Harness}
=
	ext{Semantic Session}
+
	ext{Presentation Grammar}
+
	ext{Reaction Policy}
+
	ext{Generated Instruments}
]
...
If it needs a core patch, classify why:
...
Do not automatically add a feature. First determine which category it is.
