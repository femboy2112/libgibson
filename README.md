...
Core engine behavior is **IMPLEMENTED + TESTED on Linux x86_64 only**. The repository currently runs **233 tests**: 122 library unit tests and 111 integration tests (across `capability_fallback`, `commit_invariance`, `compositor`, `demo_render`, `diff_golden`, `effects_perf`, `ffi_lifecycle`, `non_tty_redirection`, `pty_integration`, `pty_resize_torture`, `resize_torture`, `safety_api`, `scene`, `screen_state_vt100`, `structured_output`, `visual_goldens`, and `whole_renderer_vt100`).
...
- **FX substrate** (TESTED): deterministic `geom` 3D wireframe projector (cube/octahedron/torus, near-plane clipping), seeded `particles`, procedural `field` (plasma/interference + heat ramp), grapheme-safe `transition` helpers (type-on/dissolve/scramble), and safe `glitch` primitives (row shift/tear/invert/scramble) that mutate only cell content.
- **Damage inspection** (TESTED): `SurfaceDiff::dirty_cells()` + `Renderer::capture_damage`/`Context::last_dirty_cells()` expose per-frame dirty coordinates for debug overlays (the demos visualize the renderer's own damage).
...
# Run the full test suite (233 tests: 122 unit + 111 integration)
cargo test
